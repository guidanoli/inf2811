...
Proof.
  ...
  - inversion Hy2; subst.
    + inversion Hy1.
    + apply IHHy1 in H2 as H3. subst. trivial.
    + inversion H1. subst. inversion Hy1.
  - inversion Hy2; subst.
    + inversion Hy1.
    + inversion H. subst. inversion H3.
    + f_equal. apply IHHy1. apply H4.
Qed.
...
Proof.
  ...
  split.
  + apply v_funny.
  + intros H. unfold normal_form in H.
    apply H. exists (C 1). constructor.
Qed.
...
Proof.
  ...
  - constructor.
  - intros H. unfold normal_form in H.
    apply H. exists (P (C 1) (C 0)). constructor.
Qed.
...
Proof.
  ...
  generalize dependent y2.
  induction Hy1; intros y2 Hy2;
  try (inversion Hy2; subst; try trivial; inversion H3).
  inversion Hy2; subst; try (inversion Hy1; fail).
  apply IHHy1 in H3. subst. trivial.
Qed.
...
(** **** Exercise: 3 stars, standard, optional (properties_of_altered_step)
    ...
    - Is the [step] relation still deterministic?  Write yes or no and
      ...
      No, because you can evaluate a test diffently.

      test (test tru tru tru) fls fls --> test tru fls fls (ST_If)
      test (test tru tru tru) fls fls --> fls (ST_Shortcut)

      Optional: prove your answer correct in Coq. *)

Theorem bool_step_not_deterministic :
  ~ deterministic step.
Proof.
  unfold deterministic.
  intros H.
  assert (H1: (test (test tru tru tru) fls fls) --> test tru fls fls). { repeat constructor. }
  assert (H2: (test (test tru tru tru) fls fls) --> fls). { repeat constructor. }
  assert (H3: test tru fls fls = fls). { eauto. }
  discriminate.
Qed.
...
(* FILL IN HERE
   - Does a strong progress theorem hold? Write yes or no and
     ...
     Yes, because the short circuit is just a shortcut for something
     that was already possible, so it don't change anything in that respect.

     Optional: prove your answer correct in Coq.
*)

Theorem bool_step_strong_progress : forall t,
  value t \/ (exists t', t --> t').
Proof.
  intros t.
  induction t;
...
(* FILL IN HERE
   - In general, is there any way we could cause strong progress to
     ...
     Yes, if we took out ST_True, we would not be able to resolve
     (test tru t2 t3) to any other term, and test is not a value.
...
Proof.
  ...
  - (* multi_refl *)
    ...
    + (* multi_refl *)
      trivial.
    + (* multi_step *)
      assert (Hcontra: exists t', x --> t'). { exists y. auto. }
      contradiction.
  - (* multi_step *)
    induction P21.
    + (* multi_refl *)
      ...
      contradiction.
    + (* multi_step *)
      assert (Hdet: deterministic step). { apply step_deterministic. }
      unfold deterministic in *.
      assert (Heqy: y = y0). { apply Hdet with x; auto. }
      apply IHP11; try auto. rewrite Heqy. auto.
Qed.
