...
(* Read: s in the place of x in <t1> results in <t2> *)
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  | s_var1 :
      substi s x (tm_var x) s
  | s_var2 (y : string) (Hy : eqb_string x y = false) :
      substi s x (tm_var y) (tm_var y)
  | s_app (t1 t2 : tm) (t1' t2' : tm)
          (H1 : substi s x t1 t1')
          (H2 : substi s x t2 t2') :
      substi s x (tm_app t1 t2) (tm_app t1' t2')
  | s_abs1 (T : ty) (t1 : tm) :
           substi s x (tm_abs x T t1) (tm_abs x T t1)
  | s_abs2 (y : string) (T : ty) (t1 t1' : tm)
           (Hy : eqb_string x y = false)
           (Ht1 : substi s x t1 t1') :
...
Lemma eqb_string_refl : forall s, eqb_string s s = true.
Proof.
  intro s.
...
Proof.
  ...
  - generalize dependent t'.
    ...
    try (subst; auto);
    try (destruct (eqb_string x0 s0) eqn:Heq;
      try (apply eqb_string_true_iff in Heq; subst; trivial; fail);
      try (subst; auto; fail)).
  - intros H. induction H; simpl;
    try (subst; auto; fail);
    try (rewrite eqb_string_refl; trivial; fail);
    try (rewrite Hy; subst; trivial).
Qed.
