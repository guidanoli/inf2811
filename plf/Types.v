...
Proof.
  ...
  - (* nvalue *)
    ...
    induction H0; intros t' Hcontra.
    + (* zro *)
      inversion Hcontra.
    + (* scc *)
      inversion Hcontra.
      subst.
      apply IHnvalue with t1'.
...
Proof.
  ...
  (* and we can deal with several impossible
     cases all at once *)
  try solve_by_invert;
  (* and some can be dealt with just by
     inverting and running auto *)
  try (inversion HE; subst; auto; fail).
  - (* T_Prd *)
    inversion HE;
    subst;
    try auto.
...
(** **** Exercise: 2 stars, standard, especially useful (subject_expansion)
   ...
   No, for example, [|- tru \in Bool], but
                    [|- test tru tru zro \in Bool] is not provable, even though
                    test tru tru zro --> tru
*)
(* Do not modify the following line: *)
...
(** **** Exercise: 2 stars, standard (variation1)
   ...
   counterexample.
      - Determinism of [step]
          remains true, because we only fiddled with the
          typing of expressions, not with how small step
          induction works.
      - Progress
          becomes false, because although [|- scc t \in Bool],
          scc t is not a value there is no t' such that scc t --> t'.
      - Preservation
          remains true, because scc t would not enter in the
          realm of expressions that can be evaluated, so there
          is no need to worry about its type being preserved.
*)
(* Do not modify the following line: *)
...
(** **** Exercise: 2 stars, standard (variation2)
   ...
   Just determinism, because now there are two ways to reduce
   this expression.

   (test tru t2 t3) --> t3 (via ST_Funny1)

   and

   (test tru t2 t3) --> t2 (via ST_IfTrue)

   And because there exists t2 and t3 such that t2 <> t3, step
   is no longer deterministic. For example...

   test tru tru fls --> fls (ST_Funny1)
   test tru tru fls --> tru (ST_IfTrue)
*)
(* Do not modify the following line: *)
...
(** **** Exercise: 2 stars, standard, optional (variation3)
   ...
   Determinism. Couterexample:
     test tru (test tru fls tru) fls --> test tru fls tru (ST_IfTrue)
     test tru (test tru fls tru) fls --> test tru fls fls (ST_Funny2)
...
(** **** Exercise: 2 stars, standard, optional (variation4)
   ...
   None, because there is no other way to reduce [prd fls] other
   than with ST_Funny3, and because [prd fls] is not a well-typed
   term, progress and preservation are not affected.
*)
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (variation5)
   ...
   Determinism is not affected, because it only affects typing.
   
   Progress is affected, because now zro can be a boolean, but
   there is no way to reduce [test zro tru fls], for example,
   even though it is well-typed as a boolean term. In other
   terms, there is no rule in [step] that allows to reduce
   [test zro tru fls --> t'], for some term [t']. Also, it is
   not a value because test _ is not a value.

   Preservation is not maintained, counterexample is
     |- pred zro \in Nat, pred zro --> zro, and |- zro \in Bool
*)
...
(** **** Exercise: 2 stars, standard, optional (variation6)
   ...
   Progress is affected, example:
   |- test (prd zro) tru fls \in Bool
   ~ (value (test (prd zro) tru fls))
   ~ (exists t', test (prd zro) tru fls --> t')

   Preservation is affected, example:
   |- pred zro \in Bool, pred zro --> zro, |- zro \in Nat
*)
(** [] *)
...
(*
    Only determinism:
      ...
      | ST_Skip : forall t, t --> t

      CE:
      pred zro --> zro (ST_PrdZro)
      pred zro --> pred zro (ST_Skip)

    Only progress:

      | T_IsFlsZero : |- iszro fls \in Bool

      CE:
      |- iszro fls \in Bool (T_IsFlsZro)
      ~ (value (iszro fls))
      ~ (exists t', iszro fls --> t')

    Only preservation:

      | T_TestBool : forall (t1 t2 t3 : tm) (T2 T3 : ty),
                   |- t1 \in Bool -> 
                   |- t2 \in T2 ->
                   |- t3 \in T3 ->
                   |- test t1 t2 t3 \in Bool

      CE:
      |- test tru zro zro \in Bool (T_TestBool)
      test tru zro zro --> zro (ST_IfTrue)
      |- zro \in Nat (T_Zro)
*)
